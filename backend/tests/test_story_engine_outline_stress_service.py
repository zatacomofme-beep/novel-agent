...
class StoryEngineOutlineStressServiceTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_outline_stress_test_reuses_existing_task_run_when_workflow_id_is_provided(self) -> None:
        project_id = uuid4()
        user_id = uuid4()
        workflow_id = "outline_stress_test:queued-task"
        fallback_result = {
            "outline_draft": {
                "level_1": [{"level": "level_1", "title": "主线圣经", "content": "主线", "status": "todo", "node_order": 1, "locked": True}],
                "level_2": [{"level": "level_2", "title": "卷一", "content": "卷一内容", "status": "todo", "node_order": 1}],
        ...
        existing_task_run = SimpleNamespace(
            task_id=workflow_id,
        ...
        with patch(
            ...
            "services.story_engine_workflow_service.get_task_run_by_task_id",
            AsyncMock(return_value=existing_task_run),
        ), patch(
            "services.story_engine_workflow_service._run_outline_stress_fallback",
            AsyncMock(return_value=fallback_result),
        ), patch(
            ...
            "services.story_engine_workflow_service.create_task_run",
            AsyncMock(),
        ) as create_task_run_mock, patch(
            "services.story_engine_workflow_service.update_task_run",
            AsyncMock(),
        ), patch(
            "services.story_engine_workflow_service._persist_workflow_task_event",
            AsyncMock(),
        ...
        create_task_run_mock.assert_not_awaited()
